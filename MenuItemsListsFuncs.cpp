...
MenuItem PROGMEM topMenu[16] = {
    ...
    {"Fx      ", gotoPatchFxMenu}, // EMenuEntryEnterHandler?
...
MenuItem PROGMEM patchResetMenu[2] = {
    {"Back    ", goUpOneMenu}, {"Reset   ", patchResetFun}};
...
bool modeOscFilter1Fun()
{
  if (myMenu.updateLeafValue)
  {
    ...
    {
    case LP:
      sprintf(myMenu.str_oledbuf, "   %s    ", " LP");
      break;
    case HP:
      sprintf(myMenu.str_oledbuf, "   %s    ", " HP");
      break;
    case BP:
      sprintf(myMenu.str_oledbuf, "   %s    ", " BP");
      break;
    case NTC:
      sprintf(myMenu.str_oledbuf, "   %s    ", "NTC");
      break;
    case THRU:
      sprintf(myMenu.str_oledbuf, "   %s    ", "OFF");
      break;
    default:
      sprintf(myMenu.str_oledbuf, "   %s    ", " LP");
      break;
...
MenuItem PROGMEM patchNoiseFilter4Menu[13] = {
    {"Back    ", goUpOneMenu}, {"Mode NF2", gotoModeNoiseFilter4Menu}, {"Freq NF2", gotoFreqNoiseFilter4Menu}, {"Q NF2   ", gotoQNoiseFilter4Menu}, {"KeyFlNF2", gotoKeyFollowNoiseFilter4Menu}, {"BrModNF2", gotoBreathModNoiseFilter4Menu}, {"BrCrvNF2", gotoBreathCurveNoiseFilter4Menu}, {"LfoF NF2", gotoLfoFreqNoiseFilter4Menu}, {"LfoD NF2", gotoLfoDepthNoiseFilter4Menu}, {"LfoBrNF2", gotoLfoBreathNoiseFilter4Menu}, {"LfoThNF2", gotoLfoThresholdNoiseFilter4Menu}, {"SwpT NF2", gotoSweepTimeNoiseFilter4Menu}, {"SwpD NF2", gotoSweepDepthNoiseFilter4Menu}};
...
bool formantSelectFun()
{
  if (myMenu.updateLeafValue)
  {
    ...
    case 0:
      ...
      break;
    case 1:
      sprintf(myMenu.str_oledbuf, "  %s", "WoodWind");
      break;
    case 2:
      sprintf(myMenu.str_oledbuf, "  %s", "String  ");
      break;
    default:
      sprintf(myMenu.str_oledbuf, "  %s", "OFF     ");
      break;
...
bool auxInFun()
{
  if (myMenu.updateLeafValue)
  {
    ...
    mix_linein = mix_linein < 0 ? 0 : mix_linein >= 100 ? 100
                                                        : mix_linein;
...
bool breathCCFun()
{
  if (myMenu.updateLeafValue)
  {
    ...
    breath_cc += myMenu.updateLeafValue > 0   ? (breath_cc == 5 ? 2 : 1)
                 : myMenu.updateLeafValue < 0 ? (breath_cc == 7 ? -2 : -1)
                                              : 0;
...
MenuList listOctOsc1Menu(octOsc1Menu, 1);
...
bool goUpOneMenu()
{
  ...
  // display.display(); // refresh display
  display.sendBuffer();
  if (!myMenu.previousMenuStack.empty() && !myMenu.previousItemIndexStack.empty())
  {
...
bool gotoOctOsc1Menu()
{
  myMenu.previousMenuStack.push(myMenu.currentMenu);
  myMenu.previousItemIndexStack.push(myMenu.currentItemIndex);
  myMenu.setCurrentMenu(&listOctOsc1Menu);
  myMenu.knobAcceleration = 1;
  sprintf(myMenu.str_oledbuf, "   %03d    ", current_patch.nrpn_msb_osc1[CCOCTOSC1] - 64);
  display.clearBuffer(); // erase display
  display.drawStr(0, 2 * display.getMaxCharHeight(), myMenu.str_oledbuf); // 2nd line
