...
char str_buf1[64] = {0};
// naming buffer
char namebuf[31 + 1]; // TODO: max Name width based on font
bool namingFinishedFlag = false;
...
bool updateCalibrationFlag = false;
...
// non-volatile globals synth settings
uint8_t vol = 75;
int eeprom_vol = 75;
float volf = 1.5;
uint8_t fxSourcePatch = 1; // 0 = global; 1 = patch
int eeprom_fxSourcePatch = 1;
uint8_t mix_linein = 0;
int eeprom_mix_linein = 0;
float mix_lineinf = 0.0;
uint8_t FineTuneCents = 64; // 0 to 127 100 cents per halfstep, FineTune Control +/- 63/-64
int eeprom_FineTuneCents = 0;
float FineTuneCentsf = 0.0;
uint8_t Transpose = 64; // 52 to 76 semitones +/- 12
int eeprom_Transpose = 0;
float Transposef = 0.0;
uint8_t Octave = 64; // 62 to 66 octaves  +/- 2
int eeprom_Octave = 0;
float Octavef = 0.0;
...
int paste_patchNumber_prev = 0; // to detect changes
int swap_patchNumber = 0;
int swap_patchNumber_prev = 0; // to detect changes
...
float BreathCurveOscFilter1 = 1.0;    // 72,11,0,127,  using BreathOscFilterCurveLines1 instead
...
float BreathCurveOscFilter2 = 1.0;       // 73,11,0,127, using BreathOscFilterCurveLines2 instead
...
float BreathCurveNoiseFilter3 = 1.0;    // 74,11,0,127, using BreathNoiseFilterCurveLines3 instead
...
float BreathCurveNoiseFilter4 = 1.0;       // 75,11,0,127, using BreathNoiseFilterCurveLines4 instead
CurveLines BreathNoiseFiltCurveLines4 = {0.0f, 1.0f, 1.0f};
float KeyFollowPreNoiseFilter = 12; // Calibration: match 4000s
float keyfollowFilterPreNoise = 1.0;
float FreqPreNoiseFilter = 4000.0f; // Calibration: match 4000s
...
bool LinkOscFilters = true;       // 81,3,1,2,//1=Link on 2= Link off
bool LinkNoiseFilters = true;     // 81,4,1,2,
...
float maxSweepTimeOsc = 500.0;        // Calibration: set this to match 4000s
float sweepTimeOscGamma = 3.50;       // Calibration: adjust this to match 4000s
float maxSweepTimeOscFilter = 1000.0; // 255.0;
float sweepTimeOscFilterGamma = 4.0;  // Calibration: adjust this to match 4000s
float maxSweepTimeNoiseFilter = 750.0;
float sweepTimeNoiseFilterGamma = 4.0; // Calibration: adjust this to match 4000s
float maxSweepDepthOscFilter = 0.70;   // 72,9,0,127, Calibrate to match 4000s (max filter modulation is 8 octaves)
...
float offsetNoteKeyfollow = 72;                // 60.0;              // 84 = C6, 72 = C5, 60 = C4
float offsetNoteKeyfollowFilter5 = 40.0;       // 84 = C6, 72 = C5, 60 = C4
float offsetNoteKeyfollowNoise = 72;           // 60.0;         // 84 = C6, 72 = C5, 60 = C4
...
float FreqPreNoiseVariableFreq = 261.63f; // Calibration: match 4000s
float maxDelayDamp = 3000.0;              // Calibration: match 4000s
float maxDelayLevel = 0.15;               // Calibration: match 4000s
float gammaDelayLevel = 3.0;              // Calibration: match 4000s
...
patch_t current_patch = {
    ...
    {0x7f, 0x6c, 0x55, 0x16, 0x3a}}; // 154 bytes long
